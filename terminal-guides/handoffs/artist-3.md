...
- Developer 2 inventory 인계: `TEX-BEER-LIQUID`는 `hidden`, `liquid-70-with-foam`,
  `liquid-100-with-foam`; `VFX-BEER-CORE`는 `none`, `foam-settling`, `foam-crown`,
  `overflow`, `finished-steam`의 required runtime visual이다. `FD-BEER-SERVED`는 준비 목록 DOM과
