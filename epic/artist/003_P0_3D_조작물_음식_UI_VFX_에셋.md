...
- `spec/system/SYS-003_전체_게임_런타임_저장_성능.md` - `SYS-003` - `v1.0.0`
- `spec/system/SYS-004_app_프로젝트와_소스코드_아키텍처.md` - `SYS-004` - `v1.0.0`

## 목표

네기마·모모·생맥주를 직접 조작하고 품질·경고·반응을 식별하는 데 필요한 모든 P0 GLB, 재질 텍스처, 음식 표시, UI glyph와 VFX atlas를 제작해 개발자가 임시 PNG·도형을 만들지 않고 장면에 연결할 수 있게 한다.
...
1. 작업 001의 manifest와 작업 002의 소실점·광원·anchor를 제작 기준으로 고정한다.
2. 기존 음식 PNG 4종을 형태·팔레트 참고로만 사용해 꼬치·닭·대파를 모듈식 GLB로 만든다.
3. 석쇠, 집게, 부채, 붓, 맥주잔, 단일 레버와 tray를 각 2,000 triangles 이하로 제작한다.
...
- 구현 기준 spec 버전: `ART-003 v1.1.0`, `GPL-004 v1.0.0`, `UI-002 v1.0.0`, `SYS-003 v1.0.0`, `SYS-004 v1.0.0`
- 구현 기준 태스크 버전: `v1.0.0`
