...
- Artist 3: D1 드링크·서빙·좌석 정리·엑스트라·정산과 후속 D2~D4 서비스·즉시 제공·음료 화면
...
- D4 이후에는 현재 자동 검증 파이프라인을 실제 인게임 적용의 완료 증거로 사용하지 않는다. 자동 결과는 보조 신호이며, 사람이 해당 영업일을 일반 UI로 직접 플레이하고 명시적으로 `PASS`해야 stage가 완료된다.
